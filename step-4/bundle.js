	...
	var app = new _vue2.default({
		...
		methods: {
			...
			login: function login() {
				_leancloudStorage2.default.User.logIn(this.formDate.username, this.formDate.password).then(function (loginUser) {
					this.currentUser = this.getCurrentUser();
				}, function (error) {
					alert('登录失败');
				});
			...
			getCurrentUser: function getCurrentUser() {
				var _AV$User$current = _leancloudStorage2.default.User.current(),
				    ...
				    username = _AV$User$current.atteributes.username;
			}
		}
